...
function createFBO(gl, w, h, intFormat, fmt, tp, flt) {
...
function createDoubleFBO(gl, w, h, intFormat, fmt, tp, flt) {
  let fbo1 = createFBO(gl, w, h, intFormat, fmt, tp, flt);
  let fbo2 = createFBO(gl, w, h, intFormat, fmt, tp, flt);
  return {
    width: w,
    height: h,
    texelSizeX: 1.0 / w,
...
export default function FluidCursor({
  ...
  useEffect(() => {
    ...
    // Build programs
    const clearProg = createProgram(gl, SHADERS.baseVertexShader, SHADERS.clearShader);
    const splatProg = createProgram(gl, SHADERS.baseVertexShader, SHADERS.splatShader);
    const advectionProg = createProgram(gl, SHADERS.baseVertexShader, SHADERS.advectionShader);
    const divergenceProg = createProgram(gl, SHADERS.baseVertexShader, SHADERS.divergenceShader);
    const curlProg = createProgram(gl, SHADERS.baseVertexShader, SHADERS.curlShader);
    const vorticityProg = createProgram(gl, SHADERS.baseVertexShader, SHADERS.vorticityShader);
    const pressureProg = createProgram(gl, SHADERS.baseVertexShader, SHADERS.pressureShader);
    const gradSubtractProg = createProgram(gl, SHADERS.baseVertexShader, SHADERS.gradientSubtractShader);
    const displayProg = createProgram(gl, SHADERS.baseVertexShader, SHADERS.displayShader);

    let density = createDoubleFBO(gl, DYE_RESOLUTION, DYE_RESOLUTION, internalFormat, format, type, filter);
    let velocity = createDoubleFBO(gl, SIM_RESOLUTION, SIM_RESOLUTION, internalFormat, format, type, filter);
    let divergence = createFBO(gl, SIM_RESOLUTION, SIM_RESOLUTION, internalFormat, format, type, gl.NEAREST);
    let curl = createFBO(gl, SIM_RESOLUTION, SIM_RESOLUTION, internalFormat, format, type, gl.NEAREST);
    let pressure = createDoubleFBO(gl, SIM_RESOLUTION, SIM_RESOLUTION, internalFormat, format, type, gl.NEAREST);
